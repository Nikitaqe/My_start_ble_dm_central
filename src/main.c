...
// Функция обратного вызова, которая будет вызвана, когда устройство обнаружит другое устройство по BLE и
// фильтры совпадения будут удовлетворены
static void scan_filter_match(
    ...
    printk("Filters matched on UUID 0x%04x.\nAddress: %s connectable: %s\n", BT_UUID_16(uuid)->val, addr,
           connectable ? "yes" : "no");
}

static void scan_connecting_error(struct bt_scan_device_info *device_info) { printk("Connecting failed\n"); }

// Функция обратного вызова, которая будет вызвана, когда устройство начинает подключаться к другому
// устройству по BLE
...
static void scan_filter_no_match(
    ...
    if (device_info->recv_info->adv_type == BT_GAP_ADV_TYPE_ADV_DIRECT_IND) {
        ...
        err = bt_conn_le_create(device_info->recv_info->addr, BT_CONN_LE_CREATE_CONN, device_info->conn_param,
                                &conn);
...
static void scan_init(void) {
    ...
    struct bt_scan_init_param scan_init = {// Если найдено совпадение, соединяемся с устройством
                                           .connect_if_match = 1,
...
static void hogp_ready_cb(struct bt_hogp *hogp) { k_work_submit(&hids_ready_work); }
...
static void hidc_write_cb(struct bt_hogp *hidc, struct bt_hogp_rep_info *rep, uint8_t err) {
...
static uint8_t capslock_read_cb(struct bt_hogp *hogp, struct bt_hogp_rep_info *rep, uint8_t err,
                                const uint8_t *data) {
    // Если произошла ошибка, выводим сообщение об ошибке
    if (err) {
...
static void capslock_write_cb(struct bt_hogp *hogp, struct bt_hogp_rep_info *rep, uint8_t err) {
    int ret;
...
static void button_capslock_rsp(void) {
    ...
    err = bt_hogp_rep_write(&hogp, hogp.rep_boot.kbd_out, capslock_write_cb, &data, sizeof(data));
    if (err) {
        printk("Keyboard data write error (err: %d)\n", err);
        return;
    }
...
static void button_handler(uint32_t button_state, uint32_t has_changed) {
    ...
    // Если кнопка "Отправить команду Caps Lock с использованием записи с ответом", выполняем отправку команды
    // Caps Lock
